...
`@db-x/runtime` is a thin re-export of `@infra-x/runtime`, so a schema written
under it also composes inside an Infra-X graph — but you don't need Infra-X to
run DB-X. The `db-x` CLI is standalone.
