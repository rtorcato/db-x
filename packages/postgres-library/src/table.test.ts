...
describe('diffTable — indexes', () => {
	...
	// A prior index with no columns is one introspection never saw authored.
	// Treating that unknown shape as "changed" would rebuild it on every apply.
	...
	it('recreates a changed index after a column drop, not before', () => {
		...
		const diff = diffTable(
			...
			[a],
			[{ name: 'idx', columns: ['a'] }],
			prior([a, col({ name: 'gone' })], [{ name: 'idx', columns: ['gone'] }])
		)
		expect(diff.sql).toEqual([
			'DROP INDEX IF EXISTS "idx"',
